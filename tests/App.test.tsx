...
describe('App', () => {
  ...
  it('shows a debug message in chat when manual memory creation is rejected by similarity', async () => {
    const user = userEvent.setup()

    await renderApp()

    await user.click(screen.getAllByRole('button', { name: 'Criar memoria' })[0])
    await user.type(screen.getByRole('textbox', { name: 'Nova memoria' }), 'Comparar cenarios agora')
    await user.click(screen.getByRole('button', { name: 'Adicionar' }))
  ...
  it('manages chat creation, rename, pinning, selection, and deletion', async () => {
    ...
    const renameInput = screen.getByRole('textbox', { name: 'Renomear conversa' })
    await user.clear(renameInput)
    await user.type(renameInput, 'Projeto de teste')
  ...
  it('saves chat renames via modal submit and cancels via cancel button', async () => {
    const user = userEvent.setup()
    ...
    const renameInputAgain = screen.getByRole('textbox', { name: 'Renomear conversa' })
    await user.clear(renameInputAgain)
    await user.type(renameInputAgain, 'Nao salvar')
    await user.click(screen.getByRole('button', { name: 'Cancelar' }))
  ...
  it('groups only the genuinely close memories when text overlap is strong enough', async () => {
    ...
    const clusterSection = screen.getByRole('region', { name: 'Agrupamentos de memorias' })
    expect(within(clusterSection).getByText(/2\s+memorias/)).toBeInTheDocument()
